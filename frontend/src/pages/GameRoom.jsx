...
const GameRoom = () => {
  ...
  useEffect(() => {
    ...
    socket.emit('room:join', { code }, (res) => {
      ...
      setRoom(res.room);
      setReadyPlayers(res.room.currentRound?.readyPlayers || []);
    ...
    socket.on('game:bettingStarted', ({ roundNumber }) => {
      setRoom((prev) =>
        prev ? { ...prev, status: 'betting', roundNumber, currentRound: { bets: [], totalBetAmount: 0 } } : prev,
      );
      setMyBets({});
      setDiceResults([]);
      setShowResult(null);
    ...
    socket.on(
      ...
      ({ userId, clearedAmount, totalBetAmount, bets, readyPlayers: updatedReadyPlayers }) => {
        ...
        if (updatedReadyPlayers) setReadyPlayers(updatedReadyPlayers);
        // Nếu là mình hủy, reset cược cá nhân
        if (userId === user?.id) {
          setMyBets({});
        }
      },
    );
    socket.on('game:readyPlayersUpdate', ({ readyPlayers: newReadyPlayers }) => {
  ...
  const handleClearBets = () => {
    if (!canBet) return;
    const socket = getSocket();
    socket.emit('game:clearBets', (res) => {
  ...
  const handlePlayerReady = () => {
    if (!canBet) return;
    const socket = getSocket();
    socket.emit('game:playerReady', (res) => {
      if (res.error) alert(res.error);
    });
  };
  const hasMyBets = Object.values(myBets).reduce((a, b) => a + b, 0) > 0;
  const isMeReady = readyPlayers.includes(user?.id);
  ...
  return (
    <div className="game-container">
      ...
      <div className="player-strip">
        {room.players.map((p) => {
          const isPReady = room.status === 'betting' && readyPlayers.includes(p.userId);
          return (
            <div
              key={p.userId}
              className={`player-chip glass ${p.userId === room.hostId ? 'is-host' : ''} ${p.userId === user?.id ? 'is-me' : ''} ${isPReady ? 'is-ready' : ''}`}
            >
              {p.userId === room.hostId && <span className="host-badge">👑</span>}
              {isPReady && <span className="ready-badge">✅</span>}
      ...
      {/* STATUS */}
      <div className={`status-banner ${room.status}`}>{statusText[room.status]}</div>
      {/* MAIN GAME AREA */}
      <div className="main-game-area">
        {/* DICE AREA */}
        <div className="dice-area glass">
          {isRolling ? (
            <>
            ...
            <>
              <div className="dice" style={{ opacity: 0.3 }}>
                🎲
              </div>
              <div className="dice" style={{ opacity: 0.3 }}>
                🎲
      ...
      <div className="bottom-action-bar">
        ...
        {canBet && (
          <div className="bet-controls glass">
            ...
            <div className="action-buttons">
              <button
                className={`btn-clear-bets ${hasMyBets && !isMeReady ? 'active' : ''}`}
                onClick={handleClearBets}
                disabled={!hasMyBets || isMeReady}
              >
                🗑️ Hủy
              </button>
              <button
                className={`btn-ready ${hasMyBets && !isMeReady ? 'active' : ''} ${isMeReady ? 'ready' : ''}`}
                onClick={handlePlayerReady}
                disabled={!hasMyBets || isMeReady}
              >
                {isMeReady ? '⏳ Đã Xong' : '✅ Xong'}
              </button>
            </div>
          </div>
        )}
        {/* HOST CONTROLS */}
        {isHost && (
          <div className="host-controls glass">
            {(room.status === 'waiting' || room.status === 'result') && (
              <button className="btn-start-bet" onClick={handleStartBetting}>
                🔓 Mở Cược
              </button>
            )}
            {room.status === 'betting' && (
              <button className="btn-close-bet" onClick={handleCloseBetting}>
                🔒 Đóng Cược & Lắc
              </button>
            )}
          </div>
        ...
        <div className="result-overlay" onClick={() => setShowResult(null)}>
          <div className="result-card glass" onClick={(e) => e.stopPropagation()}>
            ...
            <div className={`result-settlement ${showResult.type}`}>
              ...
              {showResult.type === 'neutral' &&
                (showResult.isHost ? '🏦 Không ai cược — Hòa' : '😐 Bạn không đặt ván này')}
            </div>
            ...
            <button className="btn-dismiss" onClick={() => setShowResult(null)}>
              OK
            </button>
          </div>
        </div>
